...
function handleAction(
  ...
  if (isNaN(id)) { sendError(res, 400, "invalid_lead_id", "Invalid lead ID"); return; }
  if (!getLead(id)) { sendError(res, 404, "lead_not_found", "Lead not found"); return; }

  const updated = actionFn(id);
  if (!updated) { sendError(res, 409, "follow_up_state_conflict", errorMsg); return; }
...
router.post("/api/leads/:id/follow-up/snooze", followUpActionLimiter, csrfGuard, (req: Request, res: Response) => {
  ...
  if (typeof until !== "string" || !until.trim()) {
    sendError(res, 400, "missing_snooze_until", "until is required (ISO date string)");
    return;
  }

  const snoozeDate = new Date(until);
  if (isNaN(snoozeDate.getTime())) {
    sendError(res, 400, "invalid_snooze_until", "until must be a valid ISO date string");
    return;
  }

  if (snoozeDate.getTime() <= Date.now()) {
    sendError(res, 400, "snooze_until_in_past", "Snooze date must be in the future");
    return;
  }

  const maxDate = Date.now() + 90 * 24 * 60 * 60 * 1000;
  if (snoozeDate.getTime() > maxDate) {
    sendError(res, 400, "snooze_until_too_far", "Snooze date must be within 90 days");
    return;
  }
  ...
  if (!updated) { sendError(res, 409, "follow_up_state_conflict", "Lead is not in a valid state for snoozing"); return; }
